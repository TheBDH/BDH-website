...
This folder contains much of the user-facing logic of our website. It has all the React components, build scripts, and other things needed for Chrome, or the browser of your choice, to render the website.

- The `build` folder is generated every time webpack builds the components. 
- The `config` folder should only be changed if anything related to Webpack settings needs to be updated.
- The `dist` folder manages how Webpack delivers the files.
- The `public` folder can be deleted? idk
- The `scripts` folder contains the scripts for Webpack to build the JS files that need to be delivered.
- The `src` folder has all of the components and CSS and is probably the one you'll be working with the most. Further information inside that folder.
- The `static` folder has the bundled JS generated by Webpack.

If you ever need to add in a node module to enhance functionality of the site, just add it in `package.json` or run `npm i <module_name>` from terminal, within this folder, to install the dependency. The file `package-lock.json` should not be edited directly.

Further information about how everything works is below the divider, in the official documentation for Create React App (which is how this project started).
